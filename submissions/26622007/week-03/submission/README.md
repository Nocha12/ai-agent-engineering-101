...
|파일|범위|
|---|---|
|[../results.csv](../results.csv)|본 실험 45개 실제 실행, 과제 지정 8열|
|[RESULTS_BY_RUN.md](RESULTS_BY_RUN.md)|CSV 45행을 전부 표시한 표와 실행별 콘솔 링크|
...
실제 실행기는 작업 하나마다 별도 프로세스와 `run_id`를 생성했다. 따라서 제출 CSV는 **실제 실행 1개 = 1행**이고 완료 행의 `tasks=1`이다. 각 조건·회차는 사전에 정한 동일한 5개 작업을 포함하며, 전체는 5×3×3=45행이다. 보고서의 9개 묶음은 이 행들의 집계이지, 존재하지 않는 실행 ID를 새로 만든 것이 아니다. 하위 재귀 작업 수를 `tasks`나 gold의 분모에 넣지 않는다.
...
- `unassigned`: 완료한 실행에서 담당자가 없는 루트의 수다. 이 실험에서 완료한 29개 실행에는 모두 루트 낙찰자가 있으므로 0이다. HTTP 장애 때문에 낙찰에 도달하지 못한 실행을 정상적인 무응찰로 바꾸지 않는다.
...
따라서 완료 CSV 행의 gold 합계는 **12 정답·17 오배정**이다. 실패 전 부분 배정까지 포함한 [전체 trace 분석](../extensions/peer_dag/conditions/20260922T012131-no-token-limit-9703d2/CURRENT_CODE_EVIDENCE.json)은 **12 정답·18 오배정·15 미배정**이다. 두 집계의 분모와 포함 범위가 다르다. 보고서의 9행 표는 CSV의 완료 행만 합산하고 실패 횟수를 별도 표시한다.

복구 16행은 원래 실패한 슬롯과 정확히 일치하는지 검사하며 본 실험 CSV에 합치지 않는다. 복구 중 facts 오답 1건도 실행 자체는 완료했으므로 수치를 숨기지 않는다. 산출물 생성 성공, gold 일치, facts 통과는 서로 다른 지표다.
...
# 루트 배정/하위 작업, 실패 뒤 부분 배정, 거절/재시도, 복구 분리의 회귀 검사
