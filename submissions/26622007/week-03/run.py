...
def load_config(path):
    ...
    if (provider.get("only") != ["fireworks"] or provider.get("allow_fallbacks") is not False
            or provider.get("require_parameters") is not True):
        raise ConfigurationError("pin the fireworks provider, disable fallbacks, require parameters")
