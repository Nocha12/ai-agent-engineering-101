...
(async () => {
  ...
  await chooser.setFiles(path.join(ROOT,'week-03-architecture.excalidraw'));
  ...
  if (process.argv[2] === 'share') {
    ...
    await page.waitForFunction(()=>Array.from(document.querySelectorAll('input')).some(i=>i.value.includes('excalidraw.com/#json=')),{timeout:30000});
    ...
    await verificationPage.waitForFunction(()=>{try{return JSON.parse(localStorage.getItem('excalidraw')).length===83}catch{return false}},{timeout:45000});
    const restored=await verificationPage.evaluate(()=>JSON.parse(localStorage.getItem('excalidraw')));
    const expected=JSON.parse(fs.readFileSync(path.join(ROOT,'week-03-architecture.excalidraw'),'utf8')).elements;
    ...
    await verificationPage.screenshot({path:path.join(ROOT,'architecture-shared-preview.png')});
    ...
    fs.writeFileSync(path.join(ROOT,'excalidraw-link.json'),JSON.stringify(result,null,2)+'\n');
